...
- `loki`, `promtail`, `prometheus`, `grafana`: cluster-level observability в отдельном namespace `loki-grafana`.
...
- `k8s/platform/loki-grafana`: k3s `HelmChart` resources для Loki, Promtail, Prometheus и Grafana в namespace `loki-grafana`.
